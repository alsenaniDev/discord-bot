...
export function GamesHubPage() {
  ...
  useEffect(() => { if (!identity) return; let active = true; (async () => { try { const intent = await consumePendingRouletteIntent(identity.accessToken, identity.guildId, identity.channelId); if (!active) return; if (intent?.roomId) { try { await joinRouletteRoom(identity.accessToken, intent.roomId, identity.guildId, identity.channelId); } catch { /* The room page shows the current state if the user was already joined or the room changed. */ } if (active) navigate(`/games/roulette/room/${intent.roomId}`, { replace: true, state: { entryMode: 'InitialJoin' } }); return; } const mine = await getMyActiveRouletteRoom(identity.accessToken, identity.guildId, identity.channelId); if (active && mine.hasRoom && mine.roomId) navigate(`/games/roulette/room/${mine.roomId}`, { replace: true, state: { entryMode: 'InitialLoad' } }); } catch { /* Keep hub usable if checking current room fails. */ } })(); return () => { active = false; }; }, [identity, navigate]);
