...
function GameGroup({ title, games, emptyMessage, onPlay }: { title: string; games: ActivityGame[]; emptyMessage: string; onPlay: (game: ActivityGame) => void }) {
  return <section className="game-group"><h2 className="group-title">{title}</h2>{games.length === 0 ? <div className="empty-card">{emptyMessage}</div> : <div className="game-grid">{games.map(game => <article className={`game-card ${game.isSandbox ? 'sandbox-game' : ''}`} key={`${game.id}-${game.gameVersionId ?? 'published'}`}><div className="game-icon">{game.iconUrl ? <img src={game.iconUrl} alt=""/> : '🧠'}</div><div><div className="game-card-title"><h2>{game.name}</h2>{game.isSandbox && <span className="sandbox-badge">تجريبية{game.version ? ` · v${game.version}` : ''}</span>}</div><p>{game.description || 'ابدأ التحدي واجمع النقاط.'}</p>{game.isSandbox && <small className="sandbox-warning">{game.sandboxWarning || 'هذه نسخة تجريبية وقد تحتوي على أخطاء.'}</small>}</div><button className="button primary" onClick={() => onPlay(game)}>ابدأ اللعب</button></article>)}</div>}</section>;
}

export function GamesHubPage() {
  const { context, identity } = useActivity(); const navigate = useNavigate();
  useEffect(() => { if (!identity) return; let active = true; (async () => { try { const intent = await consumePendingRouletteIntent(identity.accessToken, identity.guildId, identity.channelId); if (!active) return; if (intent?.roomId) { try { await joinRouletteRoom(identity.accessToken, intent.roomId, identity.guildId, identity.channelId); } catch { /* The room page shows the current state if the user was already joined or the room changed. */ } if (active) navigate(`/games/roulette/room/${intent.roomId}`, { replace: true, state: { entryMode: 'InitialJoin' } }); return; } const mine = await getMyActiveRouletteRoom(identity.accessToken, identity.guildId, identity.channelId); if (active && mine.hasRoom && mine.resumeAllowed !== false && mine.roomId) navigate(`/games/roulette/room/${mine.roomId}`, { replace: true, state: { entryMode: 'InitialLoad' } }); } catch { /* Keep hub usable if checking current room fails. */ } })(); return () => { active = false; }; }, [identity, navigate]);
