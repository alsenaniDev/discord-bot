...
export function RoulettePage() {
  ...
  return <main className="page"><header className="hero"><div><span className="eyebrow">لعبة جماعية</span><h1>🎡 الروليت</h1><p>أنشئ تحديًا جماعيًا وانتظر اللاعبين من روم الألعاب.</p></div><button className="button secondary" onClick={() => navigate('/games')}>مركز الألعاب</button></header>
    <section className="roulette-summary"><div className="wallet-card"><span>رصيدي</span><strong>{balance} عملة</strong></div><button className="button primary" disabled={busy} onClick={create}>{busy ? 'جاري التجهيز...' : 'إنشاء غرفة'}</button><button className="button secondary" onClick={() => navigate('/store')}>المتجر</button><button className="button secondary" onClick={() => void load()}>تحديث الغرف</button></section>
